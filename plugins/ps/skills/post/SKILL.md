...
## Strategy scorecard (`data/social/patterns.md`) — also required

`examples.md` and the `exemplar` records teach the **shape** of a good post. They do not
tell you **what** to write or **how** to close — those are aggregate facts across all posts,
and they live in `data/social/patterns.md` (regenerated by `/ps:post-pulse` from the whole
store). **Read it before drafting**, alongside the exemplars.

Use it to make the strategic choices the exemplars can't:

- **Pillar weighting** — when Pedro asks for "a week" or leaves the topic open, bias toward
  the pillars that over-perform in the scorecard, not just the cadence table.
- **CTA choice** — let the measured CTA ranking guide the close.
- **Length** — respect the length bucket that performs.

**When the scorecard contradicts a rule in this skill, the scorecard's data wins for the
choice at hand** (e.g. if it shows engagement-question underperforming a bare or dm-advisory
close, prefer the stronger close even though this skill's default leans on the question).
The data is the tie-breaker. Note the deviation in your delivery so Pedro sees it. If the
scorecard is missing, proceed on the skill's defaults and say so.
...
5. **Load the strategy scorecard.** Read `data/social/patterns.md` for the current pillar/CTA/length/day signals. Let it steer pillar weighting (open-ended asks), CTA choice, and length; when it contradicts a rule here, the data wins for that choice (see "Strategy scorecard" above). Note any deviation in delivery.
6. **Pick the hook pattern.** From the prompt, pillar, and the exemplars you just loaded, choose a hook from `references/hook-library.md`. If the prompt implies a hook, honor it.
7. **Draft.** Follow the post structure, shaped for the network, mirroring the chosen exemplar's architecture. Use the voice signatures (scaled to network). Bake in any monetization-aware rules for this slot. **Write zero em-dashes.**
8. **Self-check before delivering.** Run this silently; fix anything that fails:
   ...
   - One pillar?
   - **One close — `▶` link CTA *or* engagement question, almost never both.** Prefer the close the scorecard (`patterns.md`) currently favors for this pillar; don't reflexively default to an engagement question if the data ranks it below a dm-advisory or bare close.
...
9. **Deliver in a fenced code block** so Pedro can copy-paste cleanly. For a thread, put each tweet in the block separated by a `---` line and numbered. Follow with a 1–2 sentence note: network used (and whether it was the default), the hook choice, which exemplar you mirrored, and any tradeoffs. For a weekly batch, label each draft with its day and pillar.
...
- `data/social/patterns.md` — The aggregate **strategy scorecard**: pillar/CTA/length/day performance across all posts, the ER baseline, and where the data contradicts a rule here. Read every draft; the data is the tie-breaker.

The performance data store at `data/social/` (schema in `data/social/schema.json`) is the
living source of examples; `/ps:post-pulse` keeps it and `patterns.md` current.
